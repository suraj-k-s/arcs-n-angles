...
const ChatBox = () => {
  ...
  const fetchChat = () => {
    axios.get("http://localhost:4000/builderuserchat/" + sessionStorage.getItem("bid") + "&" + uid.uid)
      .then((response) => {
  ...
  const userbuilderchat = () => {
    ...
    axios.post("http://localhost:4000/builderuserchat/", msg).then((response) => {
      fetchChat();
      setTxtmsg("");
  ...
  const chatattachmentuserbuilder = () => {
    ...
    axios.post("http://localhost:4000/chatattachmentbuilderuser/", formData, config).then((response) => {
