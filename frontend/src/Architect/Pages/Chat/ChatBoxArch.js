...
const ChatBoxarch = () => {
  ...
  const fetchChat = () => {
    console.log("hii");
    axios.get("http://localhost:4000/architectarchchat/" + archid.archid + "&" + sessionStorage.getItem("arid"))
      .then((response) => {
  ...
  const archarchitectchat = () => {
    alert("ENtered Chat funct");
    var msg = {
      chat: txtmsg,
      archid: archid.archid,
      aid: sessionStorage.getItem("arid"),
    }
    axios.post("http://localhost:4000/archarchitectchat/", msg).then((response) => {
      fetchChat();
      setTxtmsg("");
  ...
  const chatattachmentarcharchitect = () => {
    let formData = new FormData();
    formData.append("aid", sessionStorage.getItem("arid"));
    formData.append("attachment", attachment);
    formData.append("archid", archid.archid);
    const config = {
      headers: { "content-type": "multipart/form-data" },
    };

    axios.post("http://localhost:4000/chatattachmentarcharchitect/", formData, config).then((response) => {
    ...
    <>
      <Box className='chat_container'>
        <Box className='chat_bottom'>
          ...
          <Box>
            <Button className='send_btn' onClick={archarchitectchat}>
              <SendIcon />
            </Button>
          </Box>
        </Box>
        ...
        <Fade in={open}>
          <Box className='chat_upload'>
            ...
            <Box className='btnact'>
              ...
              <Button onClick={chatattachmentarcharchitect} className='uploadbtn'>Send</Button>
            </Box>
          </Box>
        </Fade>
      </Modal>
