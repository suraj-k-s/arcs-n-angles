...
const ChatList = () => {
    ...
    const fetchChatList = () => {
        axios.get("http://localhost:4000/chatlistarch/" + sessionStorage.getItem("arid"))
            .then((response) => {
                    ...
                    <Grow in timeout={1000} key={key}>
                        <Card className='chatlist_card'>
                            <NavLink className='chatlist_link'
                                to={cl.user_id !== '' ? `/architect/chatboxuser/${cl.user_id}`
                                    : cl.builder_id !== '' ? `/architect/chatboxbuilder/${cl.builder_id}`
                                        : cl.architect_id !== '' ? `/architect/chatboxarch/${cl.architect_id}`
                                            : null
                                }
                            >
                                <CardActionArea>
