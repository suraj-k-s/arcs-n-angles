...
const UserMainPage = () => {
  ...
  const fetchProjectData = () => {
    axios.get("http://localhost:4000/Projectbuildermain/" + sessionStorage.getItem('arid')).then((response) => {
  ...
  const handleLikeProjectClick = (e) => {
    axios.get("http://localhost:4000/likecheckbuilder/" + sessionStorage.getItem('arid') + "&" + e).then((response) => {
      fetchProjectData();
    })
  }
  ...
  const handleUserCommentSubmit = (e) => {
    var dat = {
      architectid: sessionStorage.getItem('arid'),
      projectid: e,
      comment: cmt,
    };
    axios.post("http://localhost:4000/architectcommentproject/", dat).then((response) => {
