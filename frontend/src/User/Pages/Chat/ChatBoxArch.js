...
const ChatBoxArch = () => {
  ...
  const fetchChat = () => {
    axios.get("http://localhost:4000/userarchchat/" + sessionStorage.getItem("uid") + "&" + aid.aid)
      .then((response) => {
  ...
  const userarchchat = () => {
    ...
    axios.post("http://localhost:4000/userarchchat/", msg).then((response) => {
      fetchChat();
      setTxtmsg("");
  ...
  const chatattachmentuserarch = () => {
    ...
    axios.post("http://localhost:4000/chatattachmentuserarch/", formData, config).then((response) => {
    ...
    <>
      <Box className='chat_container'>
        <Box className='chat_bottom'>
          ...
          <Box>
            <Button className='send_btn' onClick={userarchchat}>
              <SendIcon />
            </Button>
          </Box>
        </Box>
        ...
        <Fade in={open}>
          <Box className='chat_upload'>
            ...
            <Box className='btnact'>
              ...
              <Button onClick={chatattachmentuserarch} className='uploadbtn'>Send</Button>
            </Box>
          </Box>
        </Fade>
      </Modal>
