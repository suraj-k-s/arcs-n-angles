...
const UserMainPage = () => {
  ...
  const fetchProjectData = () => {
    axios.get("http://localhost:4000/Project/" + sessionStorage.getItem('uid')).then((response) => {
  ...
  const handleLikeProjectClick = (e) => {
    axios.get("http://localhost:4000/likecheckuser/" + sessionStorage.getItem('uid') + "&" + e).then((response) => {
      fetchProjectData();
    })
  }
  ...
  const handleUserCommentSubmit = (e) => {
    ...
    axios.post("http://localhost:4000/usercommentproject/", dat).then((response) => {
